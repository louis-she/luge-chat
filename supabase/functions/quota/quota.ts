...
export const ASK_PACKAGES: Record<string, { asks: number; title: string }> = {
  asks_50: { asks: 50, title: '50 次问路包' },
  asks_200: { asks: 200, title: '200 次问路包' },
}

export function mockPurchaseEnabled() {
  const schema =
    Deno.env.get('AUTH_DB_SCHEMA') ?? Deno.env.get('SANDBOX_DB_SCHEMA') ?? 'dev'
  return Deno.env.get('MOCK_PURCHASE_ENABLED') === '1' || schema === 'dev'
}
...
export async function mockPurchasePackage(
  supabase: SupabaseClient,
  opts: { userId: string; packageId: string },
): Promise<QuotaStatus> {
  const pkg = ASK_PACKAGES[opts.packageId]
  if (!pkg) throw new Error('invalid package')

  const { data: user, error } = await supabase
    .from('users')
    .select('balance_asks, billing_mode, vip_expire_at')
    .eq('id', opts.userId)
    .maybeSingle()
  ...
  if (isVipActive(user)) {
    return getQuotaStatus(supabase, { userId: opts.userId, deviceKey: null })
  }

  const nextBalance = (user.balance_asks ?? 0) + pkg.asks
  const { error: updateError } = await supabase
    .from('users')
    .update({ balance_asks: nextBalance })
    .eq('id', opts.userId)
